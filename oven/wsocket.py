...
class WebSocket(object):
 ...
 def _is_valid_close_code(self,code):
  if(code<1000 or 1004<=code<=1006 or 1012<=code<=1016 or code==1100 or 2000<=code<=2999):
   return False
  return True
 ...
 def read_message(self):
  ...
  while True:
   ...
   if not length:
    ...
    if compressed:
     payload=b"".join((self.decompressor.decompress(bytes(payload)),self.decompressor.decompress(b"\0\0\xff\xff"),self.decompressor.flush(),))
 ...
 def receive(self):
  if self.closed:
   self.handler.onclose(MSG_ALREADY_CLOSED,self)
  ...
  try:
   ...
   self.handler.onclose(MSG_CLOSED,self)
 ...
 def send(self,message,binary=None,do_compress=True):
  ...
  try:
   ...
   self.handler.onclose(MSG_SOCKET_DEAD,self)
   raise WebSocketError(MSG_SOCKET_DEAD)
 def close(self,code=1000,message=b""):
  ...
  try:
   message=self._encode_bytes(message)
   self.send_frame(struct.pack("!H%ds"%len(message),code,message),opcode=OPCODE_CLOSE)
  except WebSocketError:
...
class Response(object):
 ...
 def process_response(self,allow_write=True):
  try:
   ...
   logger.debug(log)
  if not allow_write or not self.headers_sent:
   return[]
 ...
 def process_status(self,status):
  if isinstance(status,int):
   ...
   code,status=status,_HTTP_STATUS_LINES.get(int(status))
  if not 100<=code<=999:
...
class WSocketApp:
 ...
 def not_found(self,environ,start_response):
  start_response(404)
  return "<h1>Page Not Found(404)</h1><p><b>%s</b></p>"%(environ.get("PATH_INFO")+"?"+environ.get("QUERY_STRING","\b"))
 def wsgi(self,environ,start_response):
  if len(self.routes):
   for route in self.routes:
    ...
    if route.endswith("*")and environ.get("PATH_INFO","").startswith(route[:-1]):
     r=Response(environ,start_response,self.routes[route])
     return r.process_response()
 ...
 def __call__(self,environ,start_response):
  if "wsgi.websocket" in environ or environ.get("REQUEST_METHOD","")!="GET":
   ...
   return r.process_response()
  if(environ.get("HTTP_UPGRADE","").lower()!="websocket" or "upgrade" not in environ.get("HTTP_CONNECTION","").lower()):
   r=Response(environ,start_response,self.app)
   return r.process_response()
  if "HTTP_SEC_WEBSOCKET_VERSION" not in environ:
   logger.warning("No protocol defined")
   start_response("426 Upgrade Required",[("Sec-WebSocket-Version",", ".join(self.SUPPORTED_VERSIONS))],)
   return[b"No Websocket protocol version defined"]
  version=environ.get("HTTP_SEC_WEBSOCKET_VERSION")
  if version not in self.SUPPORTED_VERSIONS:
   msg="Unsupported WebSocket Version: %s"%version
   logger.warning(msg)
   start_response("400 Bad Request",[("Sec-WebSocket-Version",", ".join(self.SUPPORTED_VERSIONS))],)
   return[msg.encode()]
  ...
  if extensions:
   ...
   do_compress=False
  if PY3:
   accept=b64encode(sha1((key+self.GUID).encode("latin-1")).digest()).decode("latin-1")
  else:
...
def run(app=WSocketApp(),host="127.0.0.1",port=8086,**options):
